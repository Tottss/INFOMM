...
-- -----------------------------------------------------
-- Placeholder table for view `clinic`.`vw_appointmentdetails`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `clinic`.`vw_appointmentdetails` (`patient_name` INT, `sex` INT, `birth_date` INT, `contact_no` INT, `attending_doctor` INT, `specialization` INT, `purpose` INT, `start_datetime` INT, `end_datetime` INT, `lab_report_status` INT, `appointment_fee` INT, `lab_report_Fee` INT, `payment_status` INT);

-- -----------------------------------------------------
-- View `clinic`.`vw_appointmentdetails`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `clinic`.`vw_appointmentdetails`;
DROP VIEW IF EXISTS `clinic`.`vw_appointmentdetails` ;
USE `clinic`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `clinic`.`vw_appointmentdetails` AS select concat(coalesce(`p`.`first_name`,''),' ',(case when ((`p`.`middle_name` is not null) and (`p`.`middle_name` <> '')) then concat(`p`.`middle_name`,' ') else '' end),coalesce(`p`.`last_name`,'')) AS `patient_name`,`p`.`sex` AS `sex`,`p`.`birth_date` AS `birth_date`,`p`.`contact_no` AS `contact_no`,concat(`d`.`First_name`,' ',`d`.`middle_name`,' ',`d`.`last_name`) AS `attending_doctor`,`d`.`specialization` AS `specialization`,`a`.`purpose` AS `purpose`,`a`.`start_datetime` AS `start_datetime`,`a`.`end_datetime` AS `end_datetime`,`lr`.`report_status` AS `lab_report_status`,(`a`.`total_fees` - `lr`.`lab_fees`) AS `appointment_fee`,`lr`.`lab_fees` AS `lab_report_Fee`,`a`.`payment_status` AS `payment_status` from (((`clinic`.`appointments` `a` join `clinic`.`patients` `p` on((`a`.`mrn` = `p`.`mrn`))) join `clinic`.`doctors` `d` on((`a`.`npi` = `d`.`npi`))) join `clinic`.`lab_reports` `lr` on((`a`.`lab_report_id` = `lr`.`lab_report_id`)));
